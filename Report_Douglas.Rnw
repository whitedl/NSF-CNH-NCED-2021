...
\section*{Clemson \& NCED Comparison CE Report: Douglas}

\textbf{Original Analysis} \\
\textbf{By: S. Ogletree}\\
Date: 2017-11-30 Note: Original File\\
Date: 2020-07-02 Note: Update to latest database. Updated the County CE datasets NCED\textunderscore CUCED\textunderscore 2020.gdb with latest CE shape files from the HUD project file. The original database was NCED\textunderscore CUCED\textunderscore 2018.gdb. Fixed quote issue in the supercedes column that failed a NA conversion. Created a new working folder called NCED\textunderscore 2020\textunderscore Analysis.  \\
\\
...
KNITR File: Report\textunderscore Douglas.Rnw \\
...
#names(n_ce )
DGLc_ce_df <- c_ce
st_geometry(DGLc_ce_df ) <- NULL
DGLn_ce_df <- n_ce
st_geometry(DGLn_ce_df) <- NULL
@
...
ce_count[1,2] <- nrow(DGLc_ce_df)
ce_count[2,2] <- DGLc_ce_df %>% filter(is.na(Supersedes)) %>% count() %>% pull(n)
ce_count[3,2] <- nrow(DGLn_ce_df)
...
ggplot(DGLc_ce_df , aes(as.factor(YearCEAdd))) + geom_bar(fill = "#e27202") + labs(x= "Year CE Added", title = paste0(county, "- CUCED"), subtitle = "Count of CE's added by year - all")
@
...
# plot the count by year added for Clemson
DGLc_ce_df %>%  filter(is.na(Supersedes)) %>% ggplot(aes(as.factor(YearCEAdd))) + geom_bar(fill = "#e27202") + labs(x= "Year CE Added", title = paste0(county, "- CUCED"), subtitle = "Count of CE's added by year - no supersedes")
@
...
ggplot(DGLn_ce_df, aes(as.factor(year_est))) + geom_bar(fill = "#5c8cdb") +  labs(x= "Year CE Added", title = paste0(county, "- NCED"), subtitle = "Count of CE's added by year")
@
...
c_year <- DGLc_ce_df %>% group_by(YearCEAdd) %>% summarise(nn = n()) %>% mutate(set = "Clemson") %>% rename(year_est = YearCEAdd)
# make a data frame with only the original CE's, no supersedes
c_year_sub <- DGLc_ce_df %>% filter(is.na(Supersedes)) %>% group_by(YearCEAdd) %>% summarise(nn = n()) %>% mutate(set = "Clemson") %>% rename(year_est = YearCEAdd) 
# the year and count for NCED
n_year <- DGLn_ce_df %>% group_by(year_est) %>% summarise(nn = n()) %>% mutate(set = "NCED")
...
tot_area[1,2] <- DGLc_ce_df %>% filter(is.na(Supersedes)) %>% summarize(CEArea_Ac = sum(CEArea_Ac))
tot_area[2,2] <- DGLn_ce_df %>% summarize(gis_acres = sum(gis_acres))
...
ggplot(DGLc_ce_df , aes(as.factor(YearCEAdd), CEArea_Ac)) + geom_col(fill = "#e27202") + labs(x= "Year CE Added", y = "Acres", title = paste0(county, "- CUCED"), subtitle = "CE Sum Acres Added by Year - all")
@
...
# plot the count by year added for Clemson
DGLc_ce_df %>%  filter(is.na(Supersedes)) %>% ggplot(aes(as.factor(YearCEAdd), CEArea_Ac)) + geom_col(fill = "#e27202") + labs(x= "Year CE Added", y = "Acres", title = paste0(county, "- CUCED"), subtitle = "CE Sum Acres Added by Year - No Supersedes")
@

<<plots_acres_NCED, fig.cap="NCED Total Acres by Year (omit supersedes)", echo=FALSE, results='latex', message=FALSE, warning=FALSE>>=
# plot the count by year added NCED
ggplot(DGLn_ce_df, aes(as.factor(year_est), gis_acres)) + geom_col(fill = "#5c8cdb") +  labs(x= "Year CE Added", y = "Acres", title = paste0(county, "- NCED"), subtitle = "Sum Acres of CE's added by year")
@
...
c_ce_df_box <- DGLc_ce_df %>% filter(is.na(Supersedes)) %>% select('YearCEAdd','CEArea_Ac') %>% mutate(Data = "Clemson") %>% rename(Acres = CEArea_Ac)
#Create df and rename columns for Acres NCED
n_ce_df_box <- DGLn_ce_df %>% select('year_est','gis_acres')  %>% mutate(Data = "NCED") %>% rename(YearCEAdd = year_est, Acres = gis_acres) 
...
##### CUMULATIVE COUNTS ####
# make a data frame with only the original CE's, no supersedes
c_year_sub_acres <- DGLc_ce_df %>% filter(is.na(Supersedes)) %>% group_by(YearCEAdd) %>% summarise(CEArea_Ac = sum(CEArea_Ac)) %>% mutate(set = "Clemson") %>% rename(year_est = YearCEAdd) 
# the year and count for NCED
n_year_acres <- DGLn_ce_df%>% group_by(year_est) %>% summarise(gis_acres = sum(gis_acres)) %>% mutate(set = "NCED")
# get the cumulative sum for clemson, no supersedes
...
DGL_CE_df <- DGLc_ce_df %>% filter(is.na(Supersedes)) %>% select('CEArea_Ac','CEAreaSqFt','CESize','Shape_Area','NearCECty')
DGL_CE_Table <- describe(DGL_CE_df)
DGL_CE_Table_digits <- print(DGL_CE_Table, digits=2, quiet = TRUE)

#Create a univariate table for NCED
DGL_NCED_df <- DGLn_ce_df %>% select('rep_acres','gis_acres','Shape_Area')
DGL_NCED_Table <- describe(DGL_NCED_df)
DGL_NCED_Table_digits <- print(DGL_NCED_Table, digits=2, quiet = TRUE)
@
...
bx_scaled <- ggplot(ce_cmb_all, aes(x=Data, y=Acres)) + geom_boxplot(notch = F, outlier.shape = NA) + coord_cartesian(ylim = quantile(ce_cmb_all$Acres, c(0.1, 0.95)))
