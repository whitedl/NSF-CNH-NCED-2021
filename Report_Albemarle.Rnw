...
\section*{Clemson \& NCED Comparison CE Report: Albemarle}
...
KNITR File: Report\textunderscore Albemarle.Rnw \\
...
#names(n_ce )
ALBc_ce_df <- c_ce
st_geometry(ALBc_ce_df ) <- NULL
ALBn_ce_df <- n_ce
st_geometry(ALBn_ce_df) <- NULL
@
...
ce_count[1,2] <- nrow(ALBc_ce_df)
ce_count[2,2] <- ALBc_ce_df %>% filter(is.na(Supersedes)) %>% count() %>% pull(n)
ce_count[3,2] <- nrow(ALBn_ce_df)
...
ggplot(ALBc_ce_df , aes(as.factor(YearCEAdd))) + geom_bar(fill = "#e27202") + labs(x= "Year CE Added", title = paste0(county, " - CUCED"), subtitle = "Count of CE's added by year - all") + theme(axis.text.y=element_text(size=8),axis.title.y=element_text(size=8, vjust=1.2),axis.text.x=element_text(size=8, angle=45, vjust=+1, hjust=1 ))
@
...
# plot the count by year added for Clemson
ALBc_ce_df %>%  filter(is.na(Supersedes)) %>% ggplot(aes(as.factor(YearCEAdd))) + geom_bar(fill = "#e27202") + labs(x= "Year CE Added", title = paste0(county, " - CUCED"), subtitle = "Count of CE's added by year - no supersedes")
@
...
ggplot(ALBn_ce_df, aes(as.factor(year_est))) + geom_bar(fill = "#5c8cdb") +  labs(x= "Year CE Added", title = paste0(county , " - NCED"), subtitle = "Count of CE's added by year") + theme(axis.text.y=element_text(size=8),axis.title.y=element_text(size=8, vjust=1.2),axis.text.x=element_text(size=8, angle=45, vjust=+1, hjust=1 ))
@
...
c_year <- ALBc_ce_df %>% group_by(YearCEAdd) %>% summarise(nn = n()) %>% mutate(set = "Clemson") %>% rename(year_est = YearCEAdd)
# make a data frame with only the original CE's, no supersedes
c_year_sub <- ALBc_ce_df %>% filter(is.na(Supersedes)) %>% group_by(YearCEAdd) %>% summarise(nn = n()) %>% mutate(set = "Clemson") %>% rename(year_est = YearCEAdd) 
# the year and count for NCED
n_year <- ALBn_ce_df %>% group_by(year_est) %>% summarise(nn = n()) %>% mutate(set = "NCED")
...
tot_area[1,2] <- ALBc_ce_df %>% filter(is.na(Supersedes)) %>% summarize(CEArea_Ac = sum(CEArea_Ac))
tot_area[2,2] <- ALBn_ce_df %>% summarize(gis_acres = sum(gis_acres))
...
ggplot(ALBc_ce_df , aes(as.factor(YearCEAdd), CEArea_Ac)) + geom_col(fill = "#e27202") + labs(x= "Year CE Added", y = "Acres", title = paste0(county, " - CUCED"), subtitle = "CE Sum Acres Added by Year - all") + theme(axis.text.y=element_text(size=8),axis.title.y=element_text(size=8, vjust=1.2),axis.text.x=element_text(size=8, angle=45, vjust=+1, hjust=1 ))
@
...
# plot the count by year added for Clemson
ALBc_ce_df %>%  filter(is.na(Supersedes)) %>% ggplot(aes(as.factor(YearCEAdd), CEArea_Ac)) + geom_col(fill = "#e27202") + labs(x= "Year CE Added", y = "Acres", title = paste0(county, " - CUCED"), subtitle = "CE Sum Acres Added by Year - No Supersedes") + theme(axis.text.y=element_text(size=8),axis.title.y=element_text(size=8, vjust=1.2),axis.text.x=element_text(size=8, angle=45, vjust=+1, hjust=1 ))
@

<<plots_acres_NCED, fig.cap="NCED Total Acres by Year (omit supersedes)", echo=FALSE, results='latex', message=FALSE, warning=FALSE>>=
# plot the count by year added NCED
ggplot(ALBn_ce_df, aes(as.factor(year_est), gis_acres)) + geom_col(fill = "#5c8cdb") +  labs(x= "Year CE Added", y = "Acres", title = paste0(county, " - NCED"), subtitle = "Sum Acres of CE's added by year") + theme(axis.text.y=element_text(size=8),axis.title.y=element_text(size=8, vjust=1.2),axis.text.x=element_text(size=8, angle=45, vjust=+1, hjust=1 ))
@
...
c_ce_df_box <- ALBc_ce_df %>% filter(is.na(Supersedes)) %>% select('YearCEAdd','CEArea_Ac') %>% mutate(Data = "Clemson") %>% rename(Acres = CEArea_Ac)
#Create df and rename columns for Acres NCED
n_ce_df_box <- ALBn_ce_df %>% select('year_est','gis_acres')  %>% mutate(Data = "NCED") %>% rename(YearCEAdd = year_est, Acres = gis_acres) 
...
ggplot(ce_cmb_all, aes(as.factor(YearCEAdd), Acres, fill = Data)) + geom_col() +  labs(x= "Year CE Added", y = "Acres",title = paste0(county, " Clemson & NCED"), subtitle = "CE Size by Year (Clemson omit supersedes)") + scale_fill_manual(values=c("#e27202", "#5c8cdb")) + theme(axis.text.y=element_text(size=8),axis.title.y=element_text(size=8, vjust=1.2),axis.text.x=element_text(size=8, angle=45, vjust=+1, hjust=1 ))
@
...
##### CUMULATIVE COUNTS ####
# make a data frame with only the original CE's, no supersedes
c_year_sub_acres <- ALBc_ce_df %>% filter(is.na(Supersedes)) %>% group_by(YearCEAdd) %>% summarise(CEArea_Ac = sum(CEArea_Ac)) %>% mutate(set = "Clemson") %>% rename(year_est = YearCEAdd) 
# the year and count for NCED
n_year_acres <- ALBn_ce_df%>% group_by(year_est) %>% summarise(gis_acres = sum(gis_acres)) %>% mutate(set = "NCED")
# get the cumulative sum for clemson, no supersedes
...
ALB_CE_df <- ALBc_ce_df %>% filter(is.na(Supersedes)) %>% select('CEArea_Ac','CEAreaSqFt','CESize','Shape_Area','NearCECty')
ALB_CE_Table <- describe(ALB_CE_df)
ALB_CE_Table_digits <- print(ALB_CE_Table, digits=2, quiet = TRUE)

#Create a univariate table for NCED
ALB_NCED_df <- ALBn_ce_df %>% select('rep_acres','gis_acres','Shape_Area')
ALB_NCED_Table <- describe(ALB_NCED_df)
ALB_NCED_Table_digits <- print(ALB_NCED_Table, digits=2, quiet = TRUE)
@
...
ggplot(ce_cmb_all, aes(x=Data, y=log10(Acres))) + geom_boxplot(notch = F)  + labs(title="Log(10)Acres by Clemson and NCED & Omit Supersedes")
