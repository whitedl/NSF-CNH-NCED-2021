...
KNITR File: Report\textunderscore Washington.Rnw \\
...
#names(n_ce )
WASc_ce_df <- c_ce
st_geometry(WASc_ce_df ) <- NULL
WASn_ce_df <- n_ce
st_geometry(WASn_ce_df) <- NULL
@
...
ce_count[1,2] <- nrow(WASc_ce_df)
ce_count[2,2] <- WASc_ce_df %>% filter(is.na(Supersedes)) %>% count() %>% pull(n)
ce_count[3,2] <- nrow(WASn_ce_df)
...
ggplot(WASc_ce_df , aes(as.factor(YearCEAdd))) + geom_bar(fill = "#e27202") + labs(x= "Year CE Added", title = paste0(county, "- CUCED"), subtitle = "Count of CE's added by year - all")
@
...
# plot the count by year added for Clemson
WASc_ce_df %>%  filter(is.na(Supersedes)) %>% ggplot(aes(as.factor(YearCEAdd))) + geom_bar(fill = "#e27202") + labs(x= "Year CE Added", title = paste0(county, "- CUCED"), subtitle = "Count of CE's added by year - no supersedes")
@
...
ggplot(WASn_ce_df, aes(as.factor(year_est))) + geom_bar(fill = "#5c8cdb") +  labs(x= "Year CE Added", title = paste0(county, "- NCED"), subtitle = "Count of CE's added by year")
@
...
c_year <- WASc_ce_df %>% group_by(YearCEAdd) %>% summarise(nn = n()) %>% mutate(set = "Clemson") %>% rename(year_est = YearCEAdd)
# make a data frame with only the original CE's, no supersedes
c_year_sub <- WASc_ce_df %>% filter(is.na(Supersedes)) %>% group_by(YearCEAdd) %>% summarise(nn = n()) %>% mutate(set = "Clemson") %>% rename(year_est = YearCEAdd) 
# the year and count for NCED
n_year <- WASn_ce_df %>% group_by(year_est) %>% summarise(nn = n()) %>% mutate(set = "NCED")
...
tot_area[1,2] <- WASc_ce_df %>% filter(is.na(Supersedes)) %>% summarize(CEArea_Ac = sum(CEArea_Ac))
tot_area[2,2] <- WASn_ce_df %>% summarize(gis_acres = sum(gis_acres))
...
ggplot(WASc_ce_df , aes(as.factor(YearCEAdd), CEArea_Ac)) + geom_col(fill = "#e27202") + labs(x= "Year CE Added", y = "Acres", title = paste0(county, "- CUCED"), subtitle = "CE Sum Acres Added by Year - all")
@
...
# plot the count by year added for Clemson
WASc_ce_df %>%  filter(is.na(Supersedes)) %>% ggplot(aes(as.factor(YearCEAdd), CEArea_Ac)) + geom_col(fill = "#e27202") + labs(x= "Year CE Added", y = "Acres", title = paste0(county, "- CUCED"), subtitle = "CE Sum Acres Added by Year - No Supersedes")
@

<<plots_acres_NCED, fig.cap="NCED Total Acres by Year (omit supersedes)", echo=FALSE, results='latex', message=FALSE, warning=FALSE>>=
# plot the count by year added NCED
ggplot(WASn_ce_df, aes(as.factor(year_est), gis_acres)) + geom_col(fill = "#5c8cdb") +  labs(x= "Year CE Added", y = "Acres", title = paste0(county, "- NCED"), subtitle = "Sum Acres of CE's added by year")
@
...
c_ce_df_box <- WASc_ce_df %>% filter(is.na(Supersedes)) %>% select('YearCEAdd','CEArea_Ac') %>% mutate(Data = "Clemson") %>% rename(Acres = CEArea_Ac)
#Create df and rename columns for Acres NCED
n_ce_df_box <- WASn_ce_df %>% select('year_est','gis_acres')  %>% mutate(Data = "NCED") %>% rename(YearCEAdd = year_est, Acres = gis_acres) 
...
##### CUMULATIVE COUNTS ####
# make a data frame with only the original CE's, no supersedes
c_year_sub_acres <- WASc_ce_df %>% filter(is.na(Supersedes)) %>% group_by(YearCEAdd) %>% summarise(CEArea_Ac = sum(CEArea_Ac)) %>% mutate(set = "Clemson") %>% rename(year_est = YearCEAdd) 
# the year and count for NCED
n_year_acres <- WASn_ce_df%>% group_by(year_est) %>% summarise(gis_acres = sum(gis_acres)) %>% mutate(set = "NCED")
# get the cumulative sum for clemson, no supersedes
...
WAS_CE_df <- WASc_ce_df %>% filter(is.na(Supersedes)) %>% select('CEArea_Ac','CEAreaSqFt','CESize','Shape_Area','NearCECty')
WAS_CE_Table <- describe(WAS_CE_df)
WAS_CE_Table_digits <- print(WAS_CE_Table, digits=2, quiet = TRUE)

#Create a univariate table for NCED
WAS_NCED_df <- WASn_ce_df %>% select('rep_acres','gis_acres','Shape_Area')
WAS_NCED_Table <- describe(WAS_NCED_df)
WAS_NCED_Table_digits <- print(WAS_NCED_Table, digits=2, quiet = TRUE)
@
