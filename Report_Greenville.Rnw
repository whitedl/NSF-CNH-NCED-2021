...
\section*{Clemson \& NCED Comparison CE Report: Greenville}

\textbf{Original Analysis} \\
\textbf{By: S. Ogletree}\\
Date: 2017-11-30 Note: Original File\\
Date: 2020-07-02 Note: Update to latest database. Updated the County CE datasets NCED\textunderscore CUCED\textunderscore 2020.gdb with latest CE shape files from the HUD project file. The original database was NCED\textunderscore CUCED\textunderscore 2018.gdb. Fixed quote issue in the supercedes column that failed a NA conversion see line #64. Created a new working folder called NCED\textunderscore 2020\textunderscore Analysis. Fixed a column label for Shape\textunderscore Area on line #207. \\
\\
...
KNITR File: Report\textunderscore Greenville.Rnw \\
...
#names(n_ce )
GVLc_ce_df <- c_ce
st_geometry(GVLc_ce_df ) <- NULL
GVLn_ce_df <- n_ce
st_geometry(GVLn_ce_df) <- NULL
@
...
ce_count[1,2] <- nrow(GVLc_ce_df)
ce_count[2,2] <- GVLc_ce_df %>% filter(is.na(Supersedes)) %>% count() %>% pull(n)
ce_count[3,2] <- nrow(GVLn_ce_df)
...
ggplot(GVLc_ce_df , aes(as.factor(YearCEAdd))) + geom_bar(fill = "#e27202") + labs(x= "Year CE Added", title = paste0(county, "- CUCED"), subtitle = "Count of CE's added by year - all")
@
...
# plot the count by year added for Clemson
GVLc_ce_df %>%  filter(is.na(Supersedes)) %>% ggplot(aes(as.factor(YearCEAdd))) + geom_bar(fill = "#e27202") + labs(x= "Year CE Added", title = paste0(county, "- CUCED"), subtitle = "Count of CE's added by year - no supersedes")
@
...
ggplot(GVLn_ce_df, aes(as.factor(year_est))) + geom_bar(fill = "#5c8cdb") +  labs(x= "Year CE Added", title = paste0(county, "- NCED"), subtitle = "Count of CE's added by year")
@
...
c_year <- GVLc_ce_df %>% group_by(YearCEAdd) %>% summarise(nn = n()) %>% mutate(set = "Clemson") %>% rename(year_est = YearCEAdd)
# make a data frame with only the original CE's, no supersedes
c_year_sub <- GVLc_ce_df %>% filter(is.na(Supersedes)) %>% group_by(YearCEAdd) %>% summarise(nn = n()) %>% mutate(set = "Clemson") %>% rename(year_est = YearCEAdd) 
# the year and count for NCED
n_year <- GVLn_ce_df %>% group_by(year_est) %>% summarise(nn = n()) %>% mutate(set = "NCED")
...
ggplot(yr_compare2, aes(as.factor(year_est), nn, fill = set)) + geom_bar(stat = "identity",position = "dodge") +  labs(x= "Year CE Added", y = "count",title = paste0(county, " Compare - NCED - Clemson"), subtitle = "Count of CE's added by year (Clemson omit supersedes)") + scale_fill_manual(values=c("#e27202", "#5c8cdb"))
...
tot_area[1,2] <- GVLc_ce_df %>% filter(is.na(Supersedes)) %>% summarize(CEArea_Ac = sum(CEArea_Ac))
tot_area[2,2] <- GVLn_ce_df %>% summarize(gis_acres = sum(gis_acres))
...
ggplot(GVLc_ce_df , aes(as.factor(YearCEAdd), CEArea_Ac)) + geom_col(fill = "#e27202") + labs(x= "Year CE Added", y = "Acres", title = paste0(county, "- CUCED"), subtitle = "CE Sum Acres Added by Year - all")
@
...
# plot the count by year added for Clemson
GVLc_ce_df %>%  filter(is.na(Supersedes)) %>% ggplot(aes(as.factor(YearCEAdd), CEArea_Ac)) + geom_col(fill = "#e27202") + labs(x= "Year CE Added", y = "Acres", title = paste0(county, "- CUCED"), subtitle = "CE Sum Acres Added by Year - No Supersedes")
@

<<plots_acres_NCED, fig.cap="NCED Total Acres by Year (omit supersedes)", echo=FALSE, results='latex', message=FALSE, warning=FALSE>>=
# plot the count by year added NCED
ggplot(GVLn_ce_df, aes(as.factor(year_est), gis_acres)) + geom_col(fill = "#5c8cdb") +  labs(x= "Year CE Added", y = "Acres", title = paste0(county, "- NCED"), subtitle = "Sum Acres of CE's added by year")
@
...
c_ce_df_box <- GVLc_ce_df %>% filter(is.na(Supersedes)) %>% select('YearCEAdd','CEArea_Ac') %>% mutate(Data = "Clemson") %>% rename(Acres = CEArea_Ac)
#Create df and rename columns for Acres NCED
n_ce_df_box <- GVLn_ce_df %>% select('year_est','gis_acres')  %>% mutate(Data = "NCED") %>% rename(YearCEAdd = year_est, Acres = gis_acres) 
...
ggplot(ce_cmb_all, aes(as.factor(YearCEAdd), Acres, fill = Data)) + geom_col() +  labs(x= "Year CE Added", y = "Acres",title = paste0(county, " Clemson & NCED"), subtitle = "CE Size by Year (Clemson omit supersedes)") + scale_fill_manual(values=c("#e27202", "#5c8cdb"))
@
...
##### CUMULATIVE COUNTS ####
# make a data frame with only the original CE's, no supersedes
c_year_sub_acres <- GVLc_ce_df %>% filter(is.na(Supersedes)) %>% group_by(YearCEAdd) %>% summarise(CEArea_Ac = sum(CEArea_Ac)) %>% mutate(set = "Clemson") %>% rename(year_est = YearCEAdd) 
# the year and count for NCED
n_year_acres <- GVLn_ce_df%>% group_by(year_est) %>% summarise(gis_acres = sum(gis_acres)) %>% mutate(set = "NCED")
# get the cumulative sum for clemson, no supersedes
...
GVL_CE_df <- GVLc_ce_df %>% filter(is.na(Supersedes)) %>% select('CEArea_Ac','CEAreaSqFt','CESize','Shape_Area','NearCECty')
GVL_CE_Table <- describe(GVL_CE_df)
GVL_CE_Table_digits <- print(GVL_CE_Table, digits=2, quiet = TRUE)

#Create a univariate table for NCED
GVL_NCED_df <- GVLn_ce_df %>% select('rep_acres','gis_acres','Shape_Area')
GVL_NCED_Table <- describe(GVL_NCED_df)
GVL_NCED_Table_digits <- print(GVL_NCED_Table, digits=2, quiet = TRUE)
@
...
bx_scaled <- ggplot(ce_cmb_all, aes(x=Data, y=Acres)) + geom_boxplot(notch = F, outlier.shape = NA) + coord_cartesian(ylim = quantile(ce_cmb_all$Acres, c(0.1, 0.9)))
