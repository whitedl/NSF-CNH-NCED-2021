...
\section*{Clemson \& NCED Comparison CE Report: Charleston}

\textbf{Original Analysis} \\
\textbf{By: S. Ogletree}\\
Date: 2020-07-02 Note: Update to latest database. Updated the County CE datasets NCED\textunderscore CUCED\textunderscore 2020.gdb with latest CE shape files from the HUD project file. The original database was NCED\textunderscore CUCED\textunderscore 2018.gdb. Fixed quote issue in the supercedes column that failed a NA conversion. Added supercedes NA coversion see line 64. Created a new working folder called NCED\textunderscore 2020\textunderscore Analysis.\\
\\
...
KNITR File: Report\textunderscore Charleston.Rnw \\
...
#names(n_ce )
CHSc_ce_df <- c_ce
st_geometry(CHSc_ce_df ) <- NULL
CHSn_ce_df <- n_ce
st_geometry(CHSn_ce_df) <- NULL
@
...
ce_count[1,2] <- nrow(CHSc_ce_df)
ce_count[2,2] <- CHSc_ce_df %>% filter(is.na(Supersedes)) %>% count() %>% pull(n)
ce_count[3,2] <- nrow(CHSn_ce_df)
...
ggplot(CHSc_ce_df , aes(as.factor(YearCEAdd))) + geom_bar(fill = "#e27202") + labs(x= "Year CE Added", title = paste0(county, "- CUCED"), subtitle = "Count of CE's added by year - all")
@
...
# plot the count by year added for Clemson
CHSc_ce_df %>%  filter(is.na(Supersedes)) %>% ggplot(aes(as.factor(YearCEAdd))) + geom_bar(fill = "#e27202") + labs(x= "Year CE Added", title = paste0(county, "- CUCED"), subtitle = "Count of CE's added by year - no supersedes")
@
...
ggplot(CHSn_ce_df, aes(as.factor(year_est))) + geom_bar(fill = "#5c8cdb") +  labs(x= "Year CE Added", title = paste0(county, "- NCED"), subtitle = "Count of CE's added by year")
@
...
c_year <- CHSc_ce_df %>% group_by(YearCEAdd) %>% summarise(nn = n()) %>% mutate(set = "Clemson") %>% rename(year_est = YearCEAdd)
# make a data frame with only the original CE's, no supersedes
c_year_sub <- CHSc_ce_df %>% filter(is.na(Supersedes)) %>% group_by(YearCEAdd) %>% summarise(nn = n()) %>% mutate(set = "Clemson") %>% rename(year_est = YearCEAdd) 
# the year and count for NCED
n_year <- CHSn_ce_df %>% group_by(year_est) %>% summarise(nn = n()) %>% mutate(set = "NCED")
...
tot_area[1,2] <- CHSc_ce_df %>% filter(is.na(Supersedes)) %>% summarize(CEArea_Ac = sum(CEArea_Ac))
tot_area[2,2] <- CHSn_ce_df %>% summarize(gis_acres = sum(gis_acres))
...
ggplot(CHSc_ce_df , aes(as.factor(YearCEAdd), CEArea_Ac)) + geom_col(fill = "#e27202") + labs(x= "Year CE Added", y = "Acres", title = paste0(county, "- CUCED"), subtitle = "CE Sum Acres Added by Year - all")
@
...
# plot the count by year added for Clemson
CHSc_ce_df %>%  filter(is.na(Supersedes)) %>% ggplot(aes(as.factor(YearCEAdd), CEArea_Ac)) + geom_col(fill = "#e27202") + labs(x= "Year CE Added", y = "Acres", title = paste0(county, "- CUCED"), subtitle = "CE Sum Acres Added by Year - No Supersedes")
@

<<plots_acres_NCED, fig.cap="NCED Total Acres by Year (omit supersedes)", echo=FALSE, results='latex', message=FALSE, warning=FALSE>>=
# plot the count by year added NCED
ggplot(CHSn_ce_df, aes(as.factor(year_est), gis_acres)) + geom_col(fill = "#5c8cdb") +  labs(x= "Year CE Added", y = "Acres", title = paste0(county, "- NCED"), subtitle = "Sum Acres of CE's added by year")
@
...
c_ce_df_box <- CHSc_ce_df %>% filter(is.na(Supersedes)) %>% select('YearCEAdd','CEArea_Ac') %>% mutate(Data = "Clemson") %>% rename(Acres = CEArea_Ac)
#Create df and rename columns for Acres NCED
n_ce_df_box <- CHSn_ce_df %>% select('year_est','gis_acres')  %>% mutate(Data = "NCED") %>% rename(YearCEAdd = year_est, Acres = gis_acres) 
...
##### CUMULATIVE COUNTS ####
# make a data frame with only the original CE's, no supersedes
c_year_sub_acres <- CHSc_ce_df %>% filter(is.na(Supersedes)) %>% group_by(YearCEAdd) %>% summarise(CEArea_Ac = sum(CEArea_Ac)) %>% mutate(set = "Clemson") %>% rename(year_est = YearCEAdd) 
# the year and count for NCED
n_year_acres <- CHSn_ce_df%>% group_by(year_est) %>% summarise(gis_acres = sum(gis_acres)) %>% mutate(set = "NCED")
# get the cumulative sum for clemson, no supersedes
...
CHS_CE_df <- CHSc_ce_df %>% filter(is.na(Supersedes)) %>% select('CEArea_Ac','CEAreaSqFt','CESize','Shape_Area','NearCECty')
CHS_CE_Table <- describe(CHS_CE_df)
CHS_CE_Table_digits <- print(CHS_CE_Table, digits=2, quiet = TRUE)

#Create a univariate table for NCED
CHS_NCED_df <- CHSn_ce_df %>% select('rep_acres','gis_acres','Shape_Area')
CHS_NCED_Table <- describe(CHS_NCED_df)
CHS_NCED_Table_digits <- print(CHS_NCED_Table, digits=2, quiet = TRUE)
@
